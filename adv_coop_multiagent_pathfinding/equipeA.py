    ...
    def play (self):
        ...
        for index, id_player in enumerate(self.competing_players):
            ...
            if not self.is_case_libre(self.g[(row, col)]):
                if self.table_de_collision.sauvgarder_une_colision(row, col, id_player):
                    #Colision de trop détecter
                    (prev_row, prev_col) = self.paths[index][self.i-1]
                    new_case = self.table_de_collision.peut_t_il_bouger(prev_row, prev_col, self.g)
                    if new_case != False:
                        self.paths[index].insert(self.i, new_case)
                        self.paths[index].insert(self.i+1, (prev_row, prev_col));
                    else:
                        (prev_row, prev_col) = self.paths[index][self.i-1]
            ...
            old_row, old_col = self.paths[index][self.i-1]
            self.g[(old_row,old_col)] = 1
