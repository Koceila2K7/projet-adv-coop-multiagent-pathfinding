...
def astar2(p,verbose=False,stepwise=False):
    """
    application de l'algorithme a-star
    sur un probleme donné
        """
    ...
    nodeInit = Noeud(p.init,0,None)
    frontiere = [(nodeInit.g+p.h_value(nodeInit.etat,p.but),nodeInit, 0)] 

    reserve = {}        
    bestNoeud = nodeInit
    temp = 0
    while frontiere != [] and not p.estBut(bestNoeud.etat):              
        (min_f,bestNoeud, temp) = heapq.heappop(frontiere)
           
    # VERSION 1 --- On suppose qu'un noeud en réserve n'est jamais ré-étendu
    # Hypothèse de consistence de l'heuristique        
        
        if p.immatriculation(bestNoeud.etat) not in reserve and p.not_reserved(bestNoeud.etat, temp):            
            reserve[p.immatriculation(bestNoeud.etat)] = bestNoeud.g #maj de reserve
            nouveauxNoeuds = bestNoeud.expand(p)
            for n in nouveauxNoeuds:
                f = n.g+p.h_value(n.etat,p.but)
                heapq.heappush(frontiere, (f,n, temp+1))
    ...
    for temps, etat in enumerate(path):
        (x, y) = etat
        p.getTable().reserve(x, y, temps, p.id)
        p.getTable().reserve(x, y, temps+1, p.id)

    return path# extended slice notation to reverse list



def astar3(p, startTime=0,oldPath=[],verbose=False,stepwise=False):
    """
    application de l'algorithme a-star
    sur un probleme donné
        """
    ...
    # Hypothèse de consistence de l'heuristique        
        ...
        if p.not_reserved(bestNoeud.etat, temp):            
            reserve[p.immatriculation(bestNoeud.etat)] = bestNoeud.g #maj de reserve
            nouveauxNoeuds = bestNoeud.expand(p)
            for n in nouveauxNoeuds:
                f = n.g+p.h_value(n.etat,p.but)
...
def astar4(p, noued_interdit = [], begin=0,oldPath=[],verbose=False,stepwise=False):
    ...
    nodeInit = Noeud(p.init,0,None)
    frontiere = [(nodeInit.g+p.h_value(nodeInit.etat,p.but),nodeInit, begin)] 
